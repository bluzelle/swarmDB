...
namespace
{
    ...
    const std::string TEST_VALUE = "I2luY2x1ZGUgPG1vY2tzL21vY2tfbm9kZV9iYXNlLmhwcD4NCiNpbmNsdWRlIDxtb2Nrcy9tb2NrX3Nlc3N"
                                   "pb25fYmFzZS5ocHA+DQojaW5jbHVkZSA8bW9ja3MvbW9ja19yYWZ0X2Jhc2UuaHBwPg0KI2luY2x1ZGUgPG"
                                   "1vY2tzL21vY2tfc3RvcmFnZV9iYXNlLmhwcD4NCg==";
...
class raft_crud_test : public Test
{
public:
    raft_crud_test()
    {
    ...
    void SetUp() final
    {
        ...
        this->crud = std::make_shared<bzn::raft_crud>(mock_node, mock_raft, mock_storage, mock_subscription_manager);
    ...
    std::shared_ptr<bzn::raft_crud> crud;
};


TEST_F(raft_crud_test, test_that_follower_not_knowing_leader_fails_to_create)
{
...
TEST_F(raft_crud_test, test_that_follower_knowing_leader_fails_to_create)
{
    ...
    EXPECT_CALL(*this->mock_session, send_message(An<std::shared_ptr<std::string>>(),_)).WillOnce(Invoke(
        [&](std::shared_ptr<std::string> msg, auto)
        {
            database_response resp;
            ASSERT_TRUE(resp.ParseFromString(*msg));
            EXPECT_EQ(resp.response_case(), database_response::kRedirect);
...
TEST_F(raft_crud_test, test_that_a_candidate_fails_to_create)
{
...
TEST_F(raft_crud_test, test_that_a_leader_can_create_a_new_record)
{
...
TEST_F(raft_crud_test, test_that_a_leader_fails_to_create_an_existing_record)
{
...
TEST_F(raft_crud_test, test_that_a_follower_can_read_an_existing_record)
{
    ...
    EXPECT_CALL(*this->mock_session, send_message(An<std::shared_ptr<std::string>>(),_)).WillOnce(Invoke(
        [&](std::shared_ptr<std::string> msg, auto)
        {
            database_response resp;
            ASSERT_TRUE(resp.ParseFromString(*msg));
            EXPECT_EQ(resp.read().value(), "skdif9ek34587fk30df6vm73==");
        }));
...
TEST_F(raft_crud_test, test_that_a_follower_apon_failing_to_read_suggests_leader)
{
...
TEST_F(raft_crud_test, test_that_a_candidate_read_fails)
{
...
TEST_F(raft_crud_test, test_that_a_leader_can_read_existing_record)
{
...
TEST_F(raft_crud_test, test_that_a_follower_knowing_a_leader_attempting_update_fails)
{
...
TEST_F(raft_crud_test, test_that_a_follower_not_knowing_leader_update_fails)
{
    auto request = generate_update_request(USER_UUID, "key0", "skdif9ek34587fk30df6vm73==");
...
TEST_F(raft_crud_test, test_that_a_candidate_update_fails)
{
...
TEST_F(raft_crud_test, test_that_a_leader_can_update)
{
...
TEST_F(raft_crud_test, test_that_a_leader_cannot_update_a_record_that_does_not_exist)
{
    // if key-value pair exists, must respond with OK, and start the
    // update process via RAFT, otherwise error - key does not exist.
    const std::string key{"key0"};
...
TEST_F(raft_crud_test, test_that_a_follower_not_knowing_the_leader_delete_fails)
{
...
TEST_F(raft_crud_test, test_that_a_follower_knowing_the_leader_delete_fails)
{
...
TEST_F(raft_crud_test, test_that_a_candidate_delete_fails)
{
    // must respond with error
...
TEST_F(raft_crud_test, test_that_a_leader_can_delete_an_existing_record)
{
    ...
    EXPECT_CALL(*this->mock_session, send_message(An<std::shared_ptr<std::string>>(),_)).WillOnce(Invoke(
        [&](std::shared_ptr<std::string> msg, auto)
        {
            database_response resp;
            ASSERT_TRUE(resp.ParseFromString(*msg));
            EXPECT_EQ(resp.response_case(), database_response::RESPONSE_NOT_SET);
        }));
...
TEST_F(raft_crud_test, test_that_a_leader_fails_to_delete_an_nonexisting_record)
{
    const std::string key{"key0"};
    auto request = generate_delete_request(USER_UUID, key);
...
TEST_F(raft_crud_test, test_that_a_leader_can_return_all_of_a_users_keys)
{
    ...
    EXPECT_CALL(*this->mock_session, send_message(An<std::shared_ptr<std::string>>(),_)).WillOnce(Invoke(
        [&](std::shared_ptr<std::string> msg, auto)
        {
            database_response resp;
            ASSERT_TRUE(resp.ParseFromString(*msg));
            EXPECT_EQ(resp.keys().keys().size(), int(3));
            EXPECT_EQ(resp.keys().keys(0), "key0");
            EXPECT_EQ(resp.keys().keys(1), "key1");
            EXPECT_EQ(resp.keys().keys(2), "key2");
        }));
...
TEST_F(raft_crud_test, test_that_a_follower_can_return_all_of_a_users_keys)
{
    auto request = generate_keys_request(USER_UUID);
...
TEST_F(raft_crud_test, test_that_a_follower_can_respond_to_has_command)
{
    // Ask local storage for all the keys for the user
    ...
    EXPECT_CALL(*this->mock_session, send_message(An<std::shared_ptr<std::string>>(),_)).WillOnce(Invoke(
        [&](std::shared_ptr<std::string> msg, auto)
        {
            database_response resp;
            ASSERT_TRUE(resp.ParseFromString(*msg));
            EXPECT_EQ(resp.response_case(), database_response::kHas);
...
TEST_F(raft_crud_test, test_that_a_leader_can_respond_to_has_command)
{
    // Ask local storage for all the keys for the user
    auto request = generate_has_request(USER_UUID, "key0");
...
TEST_F(raft_crud_test, test_that_leader_can_return_the_size_of_a_database)
{
    auto request = generate_size_request(USER_UUID);

    EXPECT_CALL(*this->mock_raft, get_state()).WillOnce(Return(bzn::raft_state::follower));

    EXPECT_CALL(*this->mock_storage, get_size(USER_UUID)).WillOnce(Return(std::make_pair(321,123)));
    ...
    EXPECT_CALL(*this->mock_session, send_message(An<std::shared_ptr<std::string>>(),_)).WillOnce(Invoke(
        [&](std::shared_ptr<std::string> msg, auto)
        {
            database_response resp;
            ASSERT_TRUE(resp.ParseFromString(*msg));
            EXPECT_EQ(resp.size().keys(), 321);
            EXPECT_EQ(resp.size().bytes(), 123);
        }));
...
TEST_F(raft_crud_test, test_that_follower_can_return_the_size_of_a_database)
{
    auto request = generate_size_request(USER_UUID);
...
TEST_F(raft_crud_test, test_that_a_CRUD_command_fails_when_not_given_bzn_api_or_cmd)
{
    auto request = generate_delete_request(USER_UUID, "key");
...
TEST_F(raft_crud_test, test_that_a_create_fails_if_the_value_size_exceeds_the_limit)
{
...
TEST_F(raft_crud_test, test_that_a_update_command_fails_when_the_size_of_the_value_exceeds_the_value_size_limit)
{
    // if key-value pair exists, must respond with OK, and start the
    // update process via RAFT, otherwise error - key does not exist.
...
TEST_F(raft_crud_test, test_that_subscription_and_unsubscription_requests_are_handled)
{
    // subscribe
    {
